...
app.get('/auth/callback', async function(req, res) {
  ...
  if (!code) return res.send('<h2>Código não encontrado.</h2>');
  try {
...
function buildPayload(checkout) {
  const products = (checkout.line_items || []).map(function(item) {
    return { nome: item.name, quantidade: item.quantity, preco: item.price };
  });
  return {
...
async function processCheckout(id) {
  ...
  try {
    ...
    if (sent) {
      ...
      setTimeout(function() { processedCheckouts.delete(String(id)); }, 48 * 60 * 60 * 1000);
    }
  } catch (err) {
...
async function runPolling() {
  ...
  try {
    ...
    for (var i = 0; i < checkouts.length; i++) await processCheckout(checkouts[i].id);
  } catch (err) {
...
app.post('/webhook/nuvemshop', function(req, res) {
...
app.get('/test', async function(req, res) {
  var payload = {
    ...
    loja: 'usevegh.com.br',
    checkout_id: 99999,
    comprador: { nome: 'Cliente Teste', email: 'teste@usevegh.com.br', telefone: '31999999999' },
    produtos: [{ nome: 'Produto Teste', quantidade: 2, preco: '49.90' }],
    financeiro: { subtotal: '99.80', total: '109.80', moeda: 'BRL' },
    link_recuperacao: 'https://usevegh.com.br/checkout/teste'
  };
  try {
    var response = await axios.post(LAILLA_WEBHOOK, payload, {
      headers: { 'Content-Type': 'application/json' }, timeout: 10000
    });
    res.json({ sucesso: true, status: response.status, resposta: response.data });
  } catch (err) {
    res.json({ sucesso: false, erro: err.message, resposta: err.response && err.response.data });
  }
});
